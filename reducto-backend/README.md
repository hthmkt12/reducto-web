...
2. Start the development server:
   ```bash
   pnpm run dev
   # Starts the server on port 3000 (or specified PORT)
   ```
3. Run the API contract smoke tests against the local server (defaulting to `http://127.0.0.1:3001` or set `SMOKE_BASE_URL`):
   ```bash
